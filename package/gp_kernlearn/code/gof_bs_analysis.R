...
#### Fit/Load Model ####
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
model.name <-  'model_Ycubic_tr100_legd3'
...
model_data_gen <- function() {
  ...
  t(sapply(1:n.train.datasets, function(x){model_data_gen_single()}))
}
...
obs.mat <- matrix(unlist(model$Y.train), ncol=N.x, byrow = TRUE)
...
n.boot <- 1e2
boot.sumstats <- bootstrap_stat_dist(infinity_norm_cov, model_data_gen, n.boot) # UPDATE 3/3
