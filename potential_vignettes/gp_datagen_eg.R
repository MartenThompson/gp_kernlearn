...
abc.samples <- abc.output$results[,1:6]
X.test <- matrix(runif(25, -5, 5), ncol=1)

post.ests <- posterior_test_meanvar(abc.samples, X, Y, X.test, leg_basis_maker)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
save_slug <- 'package/gp_kernlearn/vignettes/output/polymean_rbf_p1/'
dir.create(file.path(save_slug))
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
save_slug <- 'package/gp_kernlearn/vignettes/output/polymean_rbf_p1_2/'
dir.create(file.path(save_slug))
...
param_prior <- param_prior_maker(basis.dim = leg.deg+1)
garbage <- data_gen(NA, param_prior()) # just need to run this method once o/w lazy init bites us.

M <- 100
runtime <- 10
abc.output <- abc_knn_fixedrt(M, runtime, param_prior, data_gen, Y, distance, n_cores=6, k=1, packages_load=c())
saveRDS(abc.output, paste0(save_slug, 'abc_output_10min_deg3.RData'))
#abc.output <- readRDS('package/gp_kernlearn/vignettes/output/rbf_p1/abc_output_5min.RData')
...
abc.samples <- abc.output$results[,1:(leg.deg+1)]
X.test <- matrix(sort(runif(100, -5, 5)), ncol=1)
post.ests <- posterior_test_meanvar(abc.samples, X, Y, X.test, leg_basis_maker)
