...
rbf_kernel <- function(X, alpha.0, alpha.1, fuzz) {
  K <- alpha.0*exp(-(1/alpha.1)*as.matrix(dist(X, diag=TRUE, upper=TRUE))^2)
  diag(K) <- diag(K) + fuzz
  return(K)
}

matern_kernel_euc <- function(X, sigma.0, rho, nu, fuzz) {
  ...
  for (i in 1:n) {
    for (j in 1:n) {
      if (i != j) {
        gam <- (2^(1-nu))/ gamma(nu)
        const <- (sqrt(2*nu) * D[i,j] / rho)^nu
        bess <- besselK(sqrt(2*nu) * D[i,j]/rho, nu = nu)
        K[i,j] <- sigma.0*gam*const*bess  
      }
    }
  }
  diag(K) <- sigma.0 + fuzz
